...
Claude Code plugin providing 13 tier-1 skills for the **core Deriva** ecosystem (`deriva-mcp-core` + `deriva-py`). Skills are organized as Markdown documents with optional Python scripts — no package build step required.
...
│   └── marketplace.json      # Marketplace registration (lists all 13 tier-1 skills)
├── skills/                   # 13 tier-1 skills, each in its own directory
│   ├── {skill-name}/
...
The 13 tier-1 skills cover the core Deriva surface — what works on any Deriva catalog without DerivaML.
...
| `coding-guidelines` | Project setup, pyproject.toml, uv conventions, Google docstrings, ruff |
...
(A future `deriva-context` always-on skill — Phase 2 Task 2.7 of the migration plan — will add a plugin-level context blurb explaining the tier-1 / tier-2 boundary.)
