...
- **Order RIDs client-side and treat the order as meaningful.** RID ordering is server-internal mechanics, not semantics. The server sorts rows by RID to provide stable iteration order for cursor pagination — `after_rid=X` returns rows with `RID > X`, walking the table in the server's RID collation. That ordering is what makes pagination work; it carries no semantic meaning. Client code should never sort by RID to surface "newest" or "related" rows — use `RCT` for newest-first and follow the relevant FK / association table for relatedness.
