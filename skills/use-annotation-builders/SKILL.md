...
description: "Write Python scripts using type-safe annotation builder classes (Display, VisibleColumns, TableDisplay, ColumnDisplay, PseudoColumn, FacetList) for production Deriva catalog code. Use when writing Python code to configure Chaise display annotations — not when using interactive MCP tools (use customize-display for that). Triggers on: 'annotation builder', 'annotation builders', 'production deployment script for chaise', 'python annotation script', 'type-safe annotations', 'reusable catalog configuration', 'pseudo column python', 'facet list python', 'apply_annotations'."
disable-model-invocation: true
---

# Using Annotation Builder Classes

Python builder classes for constructing Deriva annotations with full type safety and IDE autocompletion. These are ideal for production code, scripts, and notebooks where you need programmatic control over Chaise display annotations.

**This skill covers the Python builder class approach.** For quick interactive setup using MCP tools (better for one-off tweaks and exploration), see `/deriva:customize-display` instead.

> **Requires:** the `deriva-ml` Python package, which ships the annotation builder classes (`from deriva_ml.model.annotations import Display, VisibleColumns, ...`). The annotations themselves are core Chaise concepts that work on any Deriva catalog, but the typed Python wrappers currently live in the deriva-ml package. If you only have `deriva-py` installed, use `/deriva:customize-display` (MCP tools — no Python imports required) or write annotations as plain dicts.
...
| Use Case | Approach |
|----------|----------|
| Interactive catalog setup | MCP tools (`set_visible_columns`, `set_row_name_pattern`, etc.) — immediate apply, via `/deriva:customize-display` |
...
- **`/deriva:customize-display`** — For applying annotations using MCP tools (without Python scripts). Use `customize-display` for quick interactive annotation changes and one-off tweaks. This skill covers the Python script-based annotation builder approach for complex bulk annotations.
